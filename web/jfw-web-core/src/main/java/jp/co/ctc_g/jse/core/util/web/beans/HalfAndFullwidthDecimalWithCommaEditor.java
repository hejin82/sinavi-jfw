 ...
 * このクラスは、正規表現 {@code ^[-－]?([0-9０-９]{0,3}([,，][0-9０-９]{3})*[.．][0-9０-９]+)$}に従って
 * カンマ付きの半角・全角のリクエスト・パラメータを受け付けることが可能ですが、Beanにバインディングされた後、例えば入力検証失敗後の入力値の表示、
 * または、入力画面後に表示する確認画面での入力値の表示はカンマ付き半角小数値となります。<br/>
 ...
 */
public class HalfAndFullwidthDecimalWithCommaEditor extends AbstractNumberEditor {

    private static final String PATTERN_STR = "^[-－]?([0-9０-９]{0,3}([,，][0-9０-９]{3})*[.．][0-9０-９]+)$";
     ...
     * デフォルトコンストラクタです。
     */
    public HalfAndFullwidthDecimalWithCommaEditor() {}
     ...
     */
    public HalfAndFullwidthDecimalWithCommaEditor(Class<? extends Number> propertyType,
    ...
    public HalfAndFullwidthDecimalWithCommaEditor(Class<? extends Number> propertyType,
        ...
        if (propertyType == null) {
            throw new InternalException(HalfAndFullwidthDecimalWithCommaEditor.class, "E-NUMBER_EDITOR#0002");
        }
