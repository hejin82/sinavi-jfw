 ...
 * このクラスは、正規表現 {@code ^[-－]?([0０]|[1-9１-９][0-9０-９]*)$} に従い、半角・全角のリクエスト・パラメータを
 * 受け付けることが可能ですが、Beanにバインディングされた後、例えば入力検証失敗後の入力値の表示、または、入力画面後に表示する
 * 確認画面での入力値の表示は半角整数値となります。<br/>
 ...
 */
public class HalfAndFullwidthNumberEditor extends AbstractNumberEditor {

    private static final String PATTERN_STR = "^[-－]?([0０]|[1-9１-９][0-9０-９]*)$";
     ...
     * デフォルトコンストラクタです。
     */
    public HalfAndFullwidthNumberEditor() {}
     ...
     */
    public HalfAndFullwidthNumberEditor(Class<? extends Number> propertyType,
    ...
    public HalfAndFullwidthNumberEditor(Class<? extends Number> propertyType,
        ...
        if (propertyType == null) {
            throw new InternalException(HalfAndFullwidthNumberEditor.class, "E-NUMBER_EDITOR#0001");
        }
